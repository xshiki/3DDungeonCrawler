...
public abstract class InventoryDisplay : MonoBehaviour
{
    ...
    protected virtual void UpdateSlot(InventorySlot updatedSlot)
    {
        foreach(var slot in SlotDictionary)
        {
            ...
            slot.Key.UpdateUISlot(updatedSlot); // slot key - ui representation
            }
        }
    }
    ...
    public void SlotClicked(InventorySlot_UI clickedUISlot)
    {
       ...
       //Click slot has item - mouse doesnt have item - then pick up item
       if(clickedUISlot.AssignedInventorySlot.ItemData != null && mouseInventoryItem.AssignedInventorySlot.ItemData == null)
        ...
        //click slot doenst hasnt item - mouse does has item - then place mouse item in empty slot
        if (clickedUISlot.AssignedInventorySlot.ItemData == null && mouseInventoryItem.AssignedInventorySlot.ItemData != null)
        {
            // check if player is holding shift key
        ...
        //check slots have an item - decide what to do
        if (clickedUISlot.AssignedInventorySlot.ItemData != null && mouseInventoryItem.AssignedInventorySlot.ItemData != null)
        {
            ...
            {
                if(leftInStack < 1){
                    SwapSlots(clickedUISlot); //Stack is full, so just swap items
                }else{//slot not full, take what's needed/remaining 
        ...
        //is the slot size + mouse stack size > slot max stack size ? if so, take form mouse
        //if different ->then swap them
